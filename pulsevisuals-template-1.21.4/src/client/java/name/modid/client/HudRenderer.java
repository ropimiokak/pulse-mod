...
public class HudRenderer implements HudRenderCallback {
    ...
    public void onHudRender(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        int activeKeyColor = 0x9000FFFF; // Бирюзовый цвет при нажатии

        // Берем динамические координаты хаба, которые можно двигать мышкой
        int hX = Modules.hudX;
        int hY = Modules.hudY;

        // 1. ТОП БАР (Перемещаемый)
        ...
        guiGraphics.drawString(client.font, watermarkText, hX + 5, hY + 5, 0xFFFFFFFF, false);
        ...
        guiGraphics.drawString(client.font, timeText, hX + 5, hY + 26, 0xFFFFFFFF, false);

        // 3. KEYSTROKES (КЛАВИШИ WASD + SPACE)
        int keySize = 18;
        int gap = 3;
        int kX = hX;
        int kY = hY + 44;
        ...
        guiGraphics.drawString(client.font, "Модули", listX + 4, listY + 4, 0x9000FFFF, false);
        
        int offset = 18;
        if (Modules.autoSprint) {
            guiGraphics.fill(listX, listY + offset, listX + 80, listY + offset + 14, backgroundColor);
            guiGraphics.drawString(client.font, "> AutoSprint", listX + 6, listY + offset + 3, 0xFFFFFFFF, false);
            offset += 16;
        }
        if (Modules.jumpCircles) {
            guiGraphics.fill(listX, listY + offset, listX + 80, listY + offset + 14, backgroundColor);
            guiGraphics.drawString(client.font, "> JumpCircles", listX + 6, listY + offset + 3, 0xFFFFFFFF, false);
            offset += 16;
        }
        if (Modules.esp) {
            guiGraphics.fill(listX, listY + offset, listX + 80, listY + offset + 14, backgroundColor);
            guiGraphics.drawString(client.font, "> ESP", listX + 6, listY + offset + 3, 0xFFFFFFFF, false);
        }
    }
}
